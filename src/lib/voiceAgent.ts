...
export const voiceAgentService = {
  ...
  async generateAIResponse(
    ...
    return geminiService.generateConversationResponse(userInput, {
      agentName: agent.name,
      conversationHistory,
      agentPersonality: agent.description || 'helpful and professional',
      currentStep: 'conversation'
    });
  },

  // Generate training data using Gemini AI
  async generateTrainingDataWithAI(
    ...
    const geminiService = getGeminiService();
    return geminiService.generateTrainingData(
  ...
  async analyzeCallWithAI(transcript: string, agentId: string) {
    const geminiService = getGeminiService();
    return geminiService.analyzeCallTranscript(transcript, agentId);
  },

  // Generate conversation flow with AI
  async generateConversationFlowWithAI(
    agentId: string,
    ...
    await this.updateConversationFlow(agentId, flow);
    return flow;
  }
};
