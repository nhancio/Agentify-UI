...
const VoiceAgentBuilder: React.FC<VoiceAgentBuilderProps> = ({ agentId, onSave }) => {
  ...
  const loadTemplate = (templateName: string) => {
    ...
    if (template) {
      setConfig(prev => ({
        ...prev,
        conversationFlow: template as ConversationStep[]
      }));
    }
  };
  ...
  const generateConversationFlow = async () => {
    if (!aiFeatures.companyInfo.trim()) {
      alert('Please provide company information to generate conversation flow');
      return;
    }
    ...
    try {
      ...
      const flow = await geminiService.generateConversationFlow(
        'voice_agent',
        aiFeatures.companyInfo,
        aiFeatures.goals
      );

      setConfig(prev => ({
        ...prev,
        conversationFlow: flow as ConversationStep[]
      }));
    } catch (error) {
      ...
      if (error instanceof Error && error.message.includes('API key')) {
        ...
        alert('Failed to generate conversation flow. Please check your Gemini API key.');
      }
    } finally {
  ...
  const generateTrainingData = async () => {
    if (!aiFeatures.companyInfo.trim()) {
      alert('Please provide company information to generate training data');
      return;
    }
    ...
    try {
      const geminiService = getGeminiService();
      const trainingData = await geminiService.generateTrainingData(
        'voice_agent',
        aiFeatures.companyInfo,
        aiFeatures.goals.join(', ')
      );
      ...
      if (error instanceof Error && error.message.includes('API key')) {
        ...
        alert('Failed to generate training data. Please check your Gemini API key.');
      }
    } finally {
      setIsGeneratingAI(false);
    }
  ...
  const testGeminiConnection = async () => {
    try {
      ...
      if (isConnected) {
        ...
        alert('Gemini AI connection failed. Please set VITE_GEMINI_API_KEY in your .env file.');
      }
    } catch (error) {
      alert('Failed to test Gemini connection. Please set VITE_GEMINI_API_KEY in your .env file.');
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
            ...
            {/* AI Features */}
            <div className="mb-6">
              <h4 className="text-sm font-medium text-gray-700 mb-3">AI-Powered Features</h4>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Enable Gemini AI</span>
                  <button
                    onClick={() => setAiFeatures(prev => ({ ...prev, enabled: !prev.enabled }))}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${aiFeatures.enabled ? 'bg-blue-600' : 'bg-gray-200'
                      }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${aiFeatures.enabled ? 'translate-x-6' : 'translate-x-1'
                        }`}
                    />
                  </button>
                </div>
                ...
                {aiFeatures.enabled && (
                  <div className="space-y-3 p-3 bg-blue-50 rounded-lg">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Company Info</label>
                      <textarea
                        value={aiFeatures.companyInfo}
                        onChange={(e) => setAiFeatures(prev => ({ ...prev, companyInfo: e.target.value }))}
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isGeneratingAI ? 'Generating...' : 'Generate Data'}
                      </button>
                    </div>

                    <button
                      onClick={testGeminiConnection}
                      className="w-full px-2 py-1 text-xs border border-blue-300 text-blue-600 rounded hover:bg-blue-50"
                    >
                ...
                <div className="space-y-4">
                  {config.conversationFlow.map((step, index) => (
                    <div
                      ...
                      className={`relative p-4 rounded-lg border-2 cursor-pointer transition-all ${selectedStep === index
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3 flex-1">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${step.type === 'greeting' ? 'bg-blue-100 text-blue-600' :
