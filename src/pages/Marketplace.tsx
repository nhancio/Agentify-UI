...
const Marketplace: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      setLoading(true);
      console.debug('[Marketplace] table called: agents');
      ...
      if (error) {
        ...
        console.debug(`[Marketplace] agents fetched: ${data?.length ?? 0}`);
        setAgents(data || []);
      }
      setLoading(false);
    };
      ...
      <div className="lg:ml-64 flex-1 p-4 sm:p-8 pt-24">
        ...
        <div className="w-full">
          {loading ? (
            ...
            <>
              {console.debug(`[Marketplace] showing agents: ${agents.length}`)}
                  ...
                  <div key={agent.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all group">
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <h3 className="text-lg font-semibold text-gray-900 mb-1">{agent.name || agent.Name}</h3>
                          <p className="text-sm text-gray-600">{agent.Createdby || agent.created_by || ''}</p>
                        </div>
